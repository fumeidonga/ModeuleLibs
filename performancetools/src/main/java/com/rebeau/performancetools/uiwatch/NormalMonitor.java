...
/**
 * 参考GT  #https://github.com/Tencent/GT#
 */
public class NormalMonitor extends AbsMonitor {
    ...
    private static volatile NormalMonitor instance = null;
    NormalMonitorBean normalMonitorBean;

    private NormalMonitor() {
        normalMonitorBean = new NormalMonitorBean();
    }

    public static NormalMonitor getInstance() {
        if (instance == null) {
            synchronized (LogMonitor.class) {
                if (instance == null) {
                    instance = new NormalMonitor();
                }
            }
        }
        return instance;
    ...
    public void start(Context context) {
        ...
        normalMonitorBean.setContext(context);
    ...
    private Runnable normalMonitorRunnable = new Runnable() {
        ...
        public void run() {
            ...
            String gtrThreads = threadId+"";

//            String content = "normalCollect" +
//                    separator + time +
//                    separator + cpuTotal +
//                    separator + cpuApp +
//                    separator + cpuThreads +
//                    separator + memory +
//                    separator + flowUpload +
//                    separator + flowDownload +
//                    separator + gtrThreads;
            normalMonitorBean.onCollectNormalInfo(time, cpuTotal, cpuApp, cpuThreads, memory, flowUpload, flowDownload, gtrThreads);
            //GTRClient.pushData(content);
//            DVLogUtils.dt(content);
