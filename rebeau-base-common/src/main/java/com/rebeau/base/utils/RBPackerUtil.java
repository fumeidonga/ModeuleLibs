...
import com.rebeau.base.bean.DVPair;
 ...
 * Time: 13:12
 */
public final class RBPackerUtil {
    private static final String TAG = "RBPackerUtil";
    private static final String EMPTY_STRING = "unkonow";
    ...
    public static synchronized String getChannelOrThrow(final Context context)
        ...
        return readChannel(new File(info.sourceDir));
    }
    ...
    static byte[] readPayloadImpl(File file, int blockId)
            throws IOException {
        ByteBuffer buffer = readBlock(file, blockId);
        if (buffer == null) {
            return null;
        }
    ...
    private static Map<Integer, ByteBuffer> readAllBlocks(final File apkFile)
        ...
        try {
            ...
            fc = raf.getChannel();
            final ByteBuffer apkSigningBlock = findApkSigningBlock(fc).getFirst();
            blocks = findIdValues(apkSigningBlock);
        } finally {
            close(fc);
            close(raf);
        }
    ...
    public static DVPair<ByteBuffer, Long> findApkSigningBlock(
    ...
    public static DVPair<ByteBuffer, Long> findApkSigningBlock(
        ...
        return DVPair.of(apkSigBlock, apkSigBlockOffset);
    }

    public static Map<Integer, ByteBuffer> findIdValues(final ByteBuffer apkSigningBlock) throws IOException {
    ...
    /**
     * Relative <em>readBlock</em> method for reading {@code size} number of bytes from the current
