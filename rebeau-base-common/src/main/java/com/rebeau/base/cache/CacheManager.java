...
public class CacheManager {
	...
	public <T> List<T> getList(Class<T> clazz, String group, int start, int count) {
		...
		if (count <= 0 || clazz == null) {
			RBLogUtil.e(TAG, "getList  count <= 0 || clazz == null >> return null;");
			return null;
		}
		...
		RBLogUtil.i(TAG, "getList  idList.size() = " + totalCount);
		if (totalCount <= 0) {
			RBLogUtil.e(TAG, "getList  totalCount <= 0 >> return null;");
			return null;
		}

		if (start >= 0) {
			RBLogUtil.i(TAG, "getList  start >= 0 >> ");

			int end = start + count;
			if (end > totalCount) {
				end = totalCount;
			}
			RBLogUtil.i(TAG, "getList  end = " + end);
			if (end <= start) {
				RBLogUtil.e(TAG, "getList  end <= start >> return null;");
				return null;
			}

			if (start > 0 || end < totalCount) {
				RBLogUtil.i(TAG, "getList  start > 0 || end < totalCount  >> idList = idList.subList(" + start + "," + end + "); >>");
		...
		RBLogUtil.i(TAG, "getList  return list; list.size() = " + list.size() + "\n>>>>>>>>>>>>>>>>>>>>>>\n\n");
	...
	public <T> void addList(Class<T> clazz, String group, LinkedHashMap<String, T> map, int pageSize) {
		if (StringUtil.isNotEmpty(group, true) == false) {
			RBLogUtil.e(TAG, "addList  StringUtil.isNotEmpty(group, true) == false >> return;");
			return;
		}
	...
	public <T> void saveList(Class<T> clazz, String group, LinkedHashMap<String, T> map, int start, int pageSize) {
		...
		if (clazz == null || map == null || map.size() <= 0) {
			RBLogUtil.e(TAG, "saveList  clazz == null || map == null || map.size() <= 0 >> return;");
			return;
		}
		...
		RBLogUtil.i(TAG, "saveList cache.getSize() = " + cache.getSize() + "; end save \n>>>>>>>>>>>>>>>>>> \n\n");
		//		}
	...
	public <T> void save(Class<T> clazz, T data, String id, String group) {
		if (data == null || StringUtil.isNotEmpty(id, true) == false) {
			RBLogUtil.e(id, "save  data == null || StringUtil.isNotEmpty(id, true) == false  >>  return;");
			return;
		}
		...
		if (sp != null) {
			...
			RBLogUtil.i(TAG, "save sp != null >> save to group");
			List<String> idList = getIdList(clazz, group);
			if (idList == null) {
				idList = new ArrayList<String>();
			}
			if (idList.contains(id) == false) {
				RBLogUtil.i(TAG, "save idList.contains(id) == false >> add");
				idList.add(0, id);
	...
	public <T> void clear(Class<T> clazz, String group, boolean removeAllInGroup) {
		RBLogUtil.i(TAG, "clear  group = " + group + "; removeAllInGroup = " + removeAllInGroup);
	...
	public void clear(SharedPreferences sp) {
		if (sp == null) {
			RBLogUtil.e(TAG, "clearList  sp == null >> return;");
			return;
		}
	...
	public <T> void remove(Class<T> clazz, String id) {
		if (clazz == null) {
			RBLogUtil.e(TAG, "remove  clazz == null >> return;");
			return;
		}
